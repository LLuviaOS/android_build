...
function print_lunch_menu()
{
    ...
    if [ "z${LLUVIA_DEVICES_ONLY}" != "z" ]; then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LLUVIA_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.lluvia.device | grep -q "$LLUVIA_BUILD");
    then
        ...
        echo "The connected device does not appear to be $LLUVIA_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.lluvia.device | grep -q "$LLUVIA_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
